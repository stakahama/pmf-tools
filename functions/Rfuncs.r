...
## make simgrid
creategrid <- function(nFactors,FPEAK,Seed,FOLDER,newsim) {
  gr <- unique(expand.grid(nFactors=nFactors,FPEAK=FPEAK,Seed=Seed))
  if(newsim) {
    mx <- 0
  } else {
  ...
  rownames(gr) <- sprintf("soln_%s",substring(1000+mx+(1:nrow(gr)),2))
  gr
}
